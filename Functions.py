...
def plot_data(window):
    ...
    try:
        ...
        if 'Bkg Y' not in window.Data['Core levels'][sheet_name]['Background'] or not window.Data['Core levels'][sheet_name]['Background']['Bkg Y']:
        ...
        if 'Bkg X' not in window.Data['Core levels'][sheet_name]['Background'] or not window.Data['Core levels'][sheet_name]['Background']['Bkg X']:
...
def create_horizontal_toolbar(window):
    ...
    save_tool = toolbar.AddTool(wx.ID_ANY, 'Save', wx.Bitmap(os.path.join(icon_path, "save-Excel.png"), wx.BITMAP_TYPE_PNG), shortHelp="Save the Fitted Results to Excel for this Core Level")
