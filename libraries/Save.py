...
def load_peaks_library_OLD(window):
    with wx.FileDialog(window, "Load peaks library",
        ...
        # Add required number of rows (2 rows per peak)
        num_peaks = len(loaded_fitting_data['Peaks'])
        window.peak_params_grid.AppendRows(num_peaks * 2)

        # Fill grid with data
        for i, (peak_label, peak_data) in enumerate(loaded_fitting_data['Peaks'].items()):
            row = i * 2
            window.peak_params_grid.SetCellValue(row, 0, chr(65 + i))  # ID
            ...
            window.peak_params_grid.SetCellValue(row, 6, f"{peak_data['Area']:.2f}")
            window.peak_params_grid.SetCellValue(row, 7, f"{peak_data.get('Sigma', 1):.2f}")
            window.peak_params_grid.SetCellValue(row, 8, f"{peak_data.get('Gamma', 0.2):.2f}")
            window.peak_params_grid.SetCellValue(row, 9, f"{peak_data.get('Skew', 0.64):.2f}")
            window.peak_params_grid.SetCellValue(row, 13, peak_data.get('Fitting Model', 'GL (Area)'))
            window.peak_params_grid.SetCellValue(row, 14, peak_data.get('Bkg Type', 'Smart'))
            window.peak_params_grid.SetCellValue(row, 15, f"{peak_data.get('Bkg Low', '')}")
            window.peak_params_grid.SetCellValue(row, 16, f"{peak_data.get('Bkg High', '')}")
            ...
            if 'Constraints' in peak_data:
                window.peak_params_grid.SetCellValue(row + 1, 2, str(peak_data['Constraints'].get('Position', '')))
                window.peak_params_grid.SetCellValue(row + 1, 3, str(peak_data['Constraints'].get('Height', '')))
                window.peak_params_grid.SetCellValue(row + 1, 4, str(peak_data['Constraints'].get('FWHM', '')))
                window.peak_params_grid.SetCellValue(row + 1, 5, str(peak_data['Constraints'].get('L/G', '')))
                window.peak_params_grid.SetCellValue(row + 1, 6, str(peak_data['Constraints'].get('Area', '')))
                window.peak_params_grid.SetCellValue(row + 1, 7, str(peak_data['Constraints'].get('Sigma', '')))
                window.peak_params_grid.SetCellValue(row + 1, 8, str(peak_data['Constraints'].get('Gamma', '')))
                window.peak_params_grid.SetCellValue(row + 1, 9, str(peak_data['Constraints'].get('Skew', '')))

        window.Data['Core levels'][sheet_name]['Fitting'] = loaded_fitting_data

        # Use existing on_sheet_selected function to properly format the grid
        from libraries.Sheet_Operations import on_sheet_selected
        on_sheet_selected(window, sheet_name)
